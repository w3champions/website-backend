...
public class RateLimitBucketService(IMemoryCache cache, ILogger<RateLimitBucketService> logger) : IRateLimitBucketService
{
    ...
    public async Task<RateLimitLease> TryAcquireAsync(string partitionKey, int hourlyLimit, int dailyLimit)
    {
        ...
        // Use a single lock to ensure atomic creation of limiters
        await _lock.WaitAsync();
        try
        {
            // Get or create hourly limiter (using synchronous GetOrCreate to avoid unnecessary async)
            var hourlyLimiter = _cache.GetOrCreate(hourlyKey, entry =>
            {
            ...
            var dailyLimiter = _cache.GetOrCreate(dailyKey, entry =>
            {
