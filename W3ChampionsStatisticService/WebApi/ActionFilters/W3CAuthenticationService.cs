...
namespace W3ChampionsStatisticService.WebApi.ActionFilters
{
    public class W3CAuthenticationService : IW3CAuthenticationService
    {
        ...
        public W3CUserAuthenticationDto GetUserByToken(string jwt)
        {
            return W3CUserAuthenticationDto.FromJWT(jwt, JwtPublicKey);
        }
    }

    public interface IW3CAuthenticationService
    {
        W3CUserAuthenticationDto GetUserByToken(string jwt);
    }

    public class W3CUserAuthenticationDto
    {
        public static W3CUserAuthenticationDto FromJWT(string jwt, string publicKey)
        {
            try
            {
                var rsa = RSA.Create();
