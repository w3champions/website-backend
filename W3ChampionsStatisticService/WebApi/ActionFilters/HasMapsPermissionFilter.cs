...
public class HasMapsPermissionFilter : IAsyncActionFilter {
    private readonly IW3CAuthenticationService _authService;

    public HasMapsPermissionFilter(IW3CAuthenticationService authService)
    {
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (queryString.AllKeys.Contains("authorization"))
        {
            try {
                ...
                var hasPermission = res.Permissions.Contains(nameof(EPermission.Maps));
