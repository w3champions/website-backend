...
/// (<c>MapServiceExtensionsTests</c>). Depends on what Program.cs registers before calling it: the two service
...
public static class MapServiceExtensions
{
    public static IServiceCollection AddMapServices(this IServiceCollection services)
    {
        ...
        // The per-fileKey lock. ONE instance per process is load-bearing: the upload service and the
        ...
        // In-flight upload bounds (one per battleTag and the process-wide cap). ONE instance per process is load-bearing, and plain
