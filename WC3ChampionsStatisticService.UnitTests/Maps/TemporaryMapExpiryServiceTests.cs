...
public class TemporaryMapExpiryServiceTests : TemporaryMapExpirySweepTestBase
{
    ...
    public async Task TheAdminJob_RunsTheSweepOnce_AndReportsItsCounts()
    {
        ...
        context.Setup(c => c.Report(2, 0, It.Is<string>(m =>
                m.Contains("deleted=1") && m.Contains("reclaimedOrphans=1") && m.Contains("protectedFiles=0")
                && m.Contains("deleteAttempts=1") && m.Contains("deferred=0") && m.Contains("failed=0") && m.Contains("purgedSpoolFiles=0")), null))
