...
namespace WC3ChampionsStatisticService.UnitTests
{
    ...
    public class RankTests : IntegrationTestBase
    {
        ...
        public async Task LoadAndSave()
        {
            ...
            var ranks = new List<Rank> { new Rank("peter#123@10_GM_1v1", 1, 12, 1456, GateWay.Usa, GameMode.GM_1v1, 0)};
            await rankRepository.InsertRanks(ranks);
            var player = PlayerOverview.Create(new List<PlayerId> { PlayerId.Create("peter#123")}, GateWay.Usa, GameMode.GM_1v1, 0);
            player.RecordWin(true, 1234);
            await playerRepository.UpsertPlayerOverview(player);
            await playerRepository.UpsertPlayerOverview(player);
            await playerRepository.UpsertPlayerOverview(player);
            var playerLoaded = await rankRepository.LoadPlayersOfLeague(1, 0, GateWay.Usa, GameMode.GM_1v1);
        ...
        public async Task LoadAndSave_NotFound()
        {
            ...
            var ranks = new List<Rank> { new Rank("peter#123@10_GM_1v1", 1, 12, 1456, GateWay.Europe, GameMode.GM_1v1, 0)};
            await rankRepository.InsertRanks(ranks);
        ...
        public async Task LoadAndSave_NotDuplicatingWhenGoingUp()
        {
            ...
            var ranks1 = new List<Rank> { new Rank("peter#123@10_GM_1v1", 1, 12, 1456, GateWay.Europe, GameMode.GM_1v1, 0)};
            var ranks2 = new List<Rank> { new Rank("peter#123@10_GM_1v1", 1, 8, 1456, GateWay.Europe, GameMode.GM_1v1, 0)};
